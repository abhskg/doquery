...
def check_columns():
    ...
    try:
        ...
        if id_column_name != 'id':
            logger.warning(f"Found column '{id_column_name}' instead of 'id', attempting to rename...")
            
            # Attempt to rename the column
            with engine.connect() as conn:
                # Using a transaction in case something goes wrong
                with conn.begin():
                    conn.execute(text(f"ALTER TABLE document RENAME COLUMN \"{id_column_name}\" TO id"))
        ...
        for model_col_lower, model_col in expected_columns.items():
            if model_col_lower != 'id':  # We already handled id
                ...
                if not col_exists:
                    logger.error(f"Column '{model_col}' from model not found in database (case-insensitive)")
                elif actual_col_name != model_col:
                    logger.warning(f"Column case mismatch: model has '{model_col}', database has '{actual_col_name}'")
