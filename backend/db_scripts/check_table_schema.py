...
def check_schema():
    ...
    try:
        ...
        if "document" in tables:
            ...
            if missing_in_db:
                logger.error(f"Columns in model but missing in database: {missing_in_db}")
        ...
        with engine.connect() as conn:
            try:
                result = conn.execute(text("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'document'"))
