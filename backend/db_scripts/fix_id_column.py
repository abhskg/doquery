...
def fix_id_column():
    ...
    try:
        ...
        with engine.connect() as conn:
            result = conn.execute(text("SELECT column_name FROM information_schema.columns WHERE table_name = 'document'"))
        ...
        for column_name in existing_columns:
            ...
            for col in col_info:
                if col['name'] == column_name:
                    ...
                    if 'constrained_columns' in pk_constraint and column_name in pk_constraint['constrained_columns']:
        ...
        if pk_uuid_columns:
            ...
            logger.info(f"Found primary key UUID column '{column_to_rename}', will rename to 'id'")
            
            # Rename the column to 'id'
            with engine.connect() as conn:
                with conn.begin():
                    conn.execute(text(f'ALTER TABLE document RENAME COLUMN "{column_to_rename}" TO "id"'))
                    logger.info(f"Column '{column_to_rename}' renamed to 'id'")
            
            # Verify the rename worked
            with engine.connect() as conn:
                result = conn.execute(text("SELECT column_name FROM information_schema.columns WHERE table_name = 'document' AND column_name = 'id'"))
                if result.scalar():
        ...
        logger.warning("No suitable UUID primary key column found. Creating new 'id' column.")
        ...
        with engine.connect() as conn:
            with conn.begin():
                ...
                conn.execute(text('ALTER TABLE document ALTER COLUMN "id" SET NOT NULL'))
                ...
                if pk_constraint and 'name' in pk_constraint and pk_constraint['name']:
                    conn.execute(text(f"ALTER TABLE document DROP CONSTRAINT IF EXISTS \"{pk_constraint['name']}\""))
                    logger.info(f"Dropped existing primary key constraint: {pk_constraint['name']}")
