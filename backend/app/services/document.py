...
class DocumentService:
    ...
    def process_document(db: Session, document: Document) -> List[DocumentChunk]:
        ...
        """
        logger.info(f"Processing document: {document.id}, filename: {document.filename}")
        
        # Use the text splitter for better chunking
        logger.debug(f"Splitting document content into chunks (size: {len(document.content)} characters)")
        ...
        logger.debug(f"Token counts calculated: {sum(token_counts) if isinstance(token_counts, list) else 0} total tokens")
        ...
        logger.info(f"Document {document.id} processing completed with {len(document_chunks)} chunks")
    ...
    def get_all_documents(db: Session, skip: int = 0, limit: int = 100) -> List[Document]:
        ...
        """
        logger.debug(f"Retrieving all documents with pagination (skip={skip}, limit={limit})")
    ...
    def delete_document(db: Session, document_id: str) -> bool:
        ...
        logger.info(f"Document {document_id} {'deleted successfully' if result > 0 else 'not found'}")
        return result > 0
