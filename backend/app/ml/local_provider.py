...
class LocalProvider(ModelProvider):
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return data.get("data", [{}])[0].get("embedding", [0.0] * self.dimension)
            else:
    ...
    def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                embeddings = [item.get("embedding", [0.0] * self.dimension) for item in data.get("data", [])]
                
                # Ensure we have the right number of embeddings
                if len(embeddings) != len(texts):
                    # Fill in missing embeddings with zeros
                    embeddings = embeddings + [[0.0] * self.dimension] * (len(texts) - len(embeddings))
