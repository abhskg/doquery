...
class TextSplitter:
    ...
    def __init__(
        ...
        logger.debug(f"Initialized TextSplitter with chunk_size={chunk_size}, chunk_overlap={chunk_overlap}")
            ...
            if paragraph_break != -1 and paragraph_break > start_idx + self.chunk_size // 2:
                ...
                if sentence_break != -1:
                    ...
                    if space_break != -1 and space_break > start_idx + self.chunk_size // 2:
                        ...
                        logger.debug(f"No clean break found, using hard cutoff at index {end_idx}")
            
            # Add the chunk
            chunks.append(text[start_idx:end_idx])
            logger.debug(f"Added chunk from index {start_idx} to {end_idx} (length: {end_idx - start_idx})")
